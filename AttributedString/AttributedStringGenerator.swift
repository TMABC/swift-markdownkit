...
open class AttributedStringGenerator {
    ...
    open class InternalHtmlGenerator: HtmlGenerator {
        ...
        open override func generate(block: MarkdownBlock, tight: Bool = false) -> String {
            switch block {
            case .list(_, _, _):
        ...
        open override func generate(textFragment fragment: MarkdownTextFragment) -> String {
            switch fragment {
    ...
    open func generate(doc: MarkdownBlock) -> NSAttributedString? {
    ...
    open func generate(block: MarkdownBlock) -> NSAttributedString? {
        return self.generateAttributedString(self.htmlGenerator.generate(block: block))
    }
    
    /// Generates an attributed string from the given Markdown blocks
    open func generate(blocks: MarkdownBlocks) -> NSAttributedString? {
